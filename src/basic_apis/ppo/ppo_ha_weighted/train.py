...
def train(cfg: DictConfig, path_manager: PathManager):
    ...
    # 可选：使用版本化资产目录（runs/<run_id> + latest）
    ...
    if asset_cfg and bool(asset_cfg.get("use_versioned_runs", False)):
        ...
        checkpoints_dir = str(Path(run_dir) / "checkpoints")
        best_dir = str(Path(run_dir) / "best_model")
        ...
        env_config.train_rl.saving.save_path = checkpoints_dir
        env_config.train_rl.evaluation.best_model_save_path = best_dir
        env_config.train_rl.logging.tensorboard_log = logs_dir
    ...
    if hasattr(rl_cfg, 'saving'):
        ...
        if asset_cfg and bool(asset_cfg.get("use_versioned_runs", False)) and bool(asset_cfg.get("update_latest", True)):
            model_root = str(asset_cfg.model_root)
            run_dir = str(Path(rl_cfg.saving.save_path).parent)
