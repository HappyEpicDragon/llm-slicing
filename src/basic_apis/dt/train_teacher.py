...
Usage:
    ...
    .pixi/envs/default/bin/python -m src.basic_apis.dt.train_teacher
"""
import os
import torch
import numpy as np
...
def train_v2(
    ...
    save_dir="data/channel_generality/dt/ppo_teacher",
    seed=42,
    device="cuda",
    extractor="mlp",
    ent_coef=0.01,
    ...
    callbacks = [
        CheckpointCallback(save_freq=10000, save_path=ckpt_dir, name_prefix="dt_teacher"),
        EvalCallback(
            eval_env,
...
def train(cfg, paths_cfg=None, workdir=None):
    """Hydra entry point: called by channel_generality.py → train_dt_teacher mode.

    Reads cfg.train_dt_teacher and delegates to train_v2().
    """
    tc = cfg.train_dt_teacher
...
if __name__ == "__main__":
    ...
    parser.add_argument("--save_dir", type=str, default="data/channel_generality/dt/ppo_teacher")
    parser.add_argument(
        "--extractor",
        type=str,
