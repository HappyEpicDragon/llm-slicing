...
Usage:
    ...
    .pixi/envs/default/bin/python -m src.basic_apis.dt.test --model_path <path>
"""
...
from src.basic_apis.dt.env import HierarchicalSlicingEnvV2, INTER_DIM, INTRA_DIM
...
def main():
    ...
    parser.add_argument("--meta_path", type=str, default="data/channel_generality/dt/dataset/metadata.json")
    ...
    parser.add_argument("--save_root", type=str, default="data/channel_generality/dt/dt_test_results")
...
def test_dt(cfg, paths_cfg=None, workdir=None):
    """Hydra entry point: called by channel_generality.py → test_dt mode.

    Uses cfg.test_dt.  Episode range defaults from env_ha.yaml "testing"
    unless test_dt.init_episode / max_episode are set.
    """
    import torch
    from omegaconf import OmegaConf
    from stable_baselines3.common.vec_env import DummyVecEnv
    ...
    model.eval()
    print(f"[test_dt] model loaded from {tc.model_path}")
    ...
    total = sum(r["combined"] for r in all_results.values())
    print(f"\n[test_dt] TOTAL combined: {total:.4f}")
    _save_json(
...
def test_dt_tiny(cfg, paths_cfg=None, workdir=None):
    """Hydra entry point: called by channel_generality.py → test_dt_tiny mode."""
    # Reuse the exact same evaluation pipeline; tiny mode is pure config routing.
    class _CfgProxy:
        pass

    cfg_proxy = _CfgProxy()
    cfg_proxy.test_dt = cfg.test_dt_tiny
    cfg_proxy.paths = cfg.get("paths", None)
    cfg_proxy.workdir = cfg.get("workdir", None)
    return test_dt(cfg_proxy, paths_cfg=paths_cfg, workdir=workdir)
