...
from src.reevo.utils.utils import *
from src.reevo.utils.llm_client.base import BaseClient
...
class ReEvo:
    def __init__(
        ...
        self.num_gpus = int(cfg.get("num_gpus", 0))
        self.per_gpu = int(cfg.get("per_gpu", 2))
        self.init_prompt()
    ...
    def evaluate_population(self, population: list[dict]) -> list[float]:
        ...
        for response_id in range(len(population)):
            ...
            if population[response_id]["code"] is None:
                ...
                continue
            pending_rids.append(response_id)

        if self.num_gpus > 0:
            max_workers = max(1, self.num_gpus * self.per_gpu)
        else:
            max_workers = self.per_gpu

        gpu_slots: queue.Queue = queue.Queue()
        if self.num_gpus > 0:
            for g in range(self.num_gpus):
                for _ in range(self.per_gpu):
                    gpu_slots.put(g)
        else:
            for _ in range(self.per_gpu):
                gpu_slots.put(None)

        def _worker_evaluate(rid: int) -> None:
            individual = population[rid]
            gpu_id = gpu_slots.get()
            process = None
            try:
                try:
                    process = self._run_code(individual, rid, gpu_id=gpu_id)
                    process.communicate(timeout=self.cfg.timeout)
                except subprocess.TimeoutExpired as e:
                    logging.info(f"Error for response_id {rid}: {e}")
                    population[rid] = self.mark_invalid_individual(population[rid], str(e))
                ...
                if traceback_msg == '':
                    try:
                        fitness_re = re.compile(r"##FITNESS##\s+([\d.eE+-]+|inf)", re.MULTILINE)
                        matches = fitness_re.findall(stdout_str)
                        if not matches:
                            raise ValueError("No ##FITNESS## line in stdout")
                        val_str = matches[-1]
                        fval = float("inf") if val_str == "inf" else float(val_str)
                        individual["obj"] = fval if self.obj_type == "min" else -fval
                        individual["exec_success"] = True
                    except Exception:
                        population[rid] = self.mark_invalid_individual(population[rid], "Invalid std out / objective value!")
                else:
                    population[rid] = self.mark_invalid_individual(population[rid], traceback_msg)

                logging.info(f"Iteration {self.iteration}, response_id {rid}: Objective value: {population[rid]['obj']}")
            finally:
                gpu_slots.put(gpu_id)

        with ThreadPoolExecutor(max_workers=max_workers) as executor:
            futures = [executor.submit(_worker_evaluate, rid) for rid in pending_rids]
    ...
    def _run_code(self, individual: dict, response_id, gpu_id=None) -> subprocess.Popen:
        """
        Write code into a file and run eval script.
        """
        logging.debug(f"Iteration {self.iteration}: Processing Code Run {response_id}")

        gpt_path = f"{self.root_dir}/problems/{self.problem}/gpt_{response_id}.py"
        with open(gpt_path, "w", encoding="utf-8") as file:
            file.writelines(individual["code"] + "\n")

        # Build env dict with optional GPU assignment
        env = os.environ.copy()
        env["REEVO_GPT_MODULE"] = f"gpt_{response_id}"
        gpu_id_final = None
        if gpu_id is not None:
            gpu_id_final = gpu_id
            env["CUDA_VISIBLE_DEVICES"] = str(gpu_id_final)
        elif self.num_gpus > 0:
            gpu_id_final = response_id % self.num_gpus
            env["CUDA_VISIBLE_DEVICES"] = str(gpu_id_final)

        if gpu_id_final is not None:
            logging.info(f"Iteration {self.iteration}: Code {response_id} → GPU {gpu_id_final}")
