...
def init_client(cfg):
    ...
    if cfg.get("model", None): # for compatibility
        ...
        if model.startswith("gpt"):
            from src.reevo.utils.llm_client.openai import OpenAIClient
            client = OpenAIClient(model, temperature)
        elif cfg.model.startswith("GLM"):
            from src.reevo.utils.llm_client.zhipuai import ZhipuAIClient
            client = ZhipuAIClient(model, temperature)
        else: # fall back to Llama API
            from src.reevo.utils.llm_client.llama_api import LlamaAPIClient
