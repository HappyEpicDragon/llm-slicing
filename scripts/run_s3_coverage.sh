...
BASE="data/channel_generality/sensitivity/s3_coverage"
DB_META="data/channel_generality/dt_v2_8exp/dataset_D-B/metadata.json"

train_and_eval() {
    local N=$1
    local GPU=$2
    local SAVE_DIR="${BASE}/scen${N}"
    local LOG="${LOG_DIR}/s3_scen${N}.log"
    : > "${LOG}"

    echo "=== train scen${N} gpu=${GPU} start $(date) ===" | tee -a "${LOG}"
    PYTHONPATH=. CUDA_VISIBLE_DEVICES=${GPU} /root/.pixi/bin/pixi run sim train_dt_v2_tiny \
        train_dt_v2_tiny.grid_id=b2_e2 \
        train_dt_v2_tiny.dataset_dir="${SAVE_DIR}" \
        train_dt_v2_tiny.save_dir="${SAVE_DIR}/model" \
    ...
    echo "=== eval scen${N} gpu=${GPU} start $(date) ===" | tee -a "${LOG}"
    PYTHONPATH=. CUDA_VISIBLE_DEVICES=${GPU} /root/.pixi/bin/pixi run sim test_dt_v2_tiny \
        test_dt_v2_tiny.grid_id=b2_e2 \
        test_dt_v2_tiny.model_path="${SAVE_DIR}/model/final_dt_v2.pth" \
        test_dt_v2_tiny.meta_path="${SAVE_DIR}/metadata.json" \
    ...
    echo "=== scen${N} ALL DONE $(date) ===" | tee -a "${LOG}"
}
...
echo "Waiting for scen1-3 (GPU 1-3)..."
wait

train_and_eval 4 1 &
echo "Waiting for scen4 (GPU 1)..."
wait

echo "=== S3 coverage sweep ALL DONE $(date) ==="
