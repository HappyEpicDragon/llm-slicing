...
echo "=== [GPU3] RETEST b2_e1 (single seed) ===" | tee -a "$LOG_DIR/b2_e1.log"
PYTHONPATH=. /root/.pixi/bin/pixi run sim test_dt_v2_tiny \
  test_dt_v2_tiny.grid_id=b2_e1 \
  test_dt_v2_tiny.model_path=data/channel_generality/dt_v2_tiny/models/b2_e1/final_dt_v2.pth \
  ...
  >> "$LOG_DIR/b2_e1.log" 2>&1

echo "=== [GPU3] START b3_e2 ===" | tee -a "$LOG_DIR/b3_e2.log"
PYTHONPATH=. /root/.pixi/bin/pixi run sim train_dt_v2_tiny \
  train_dt_v2_tiny.grid_id=b3_e2 \
  ...
  >> "$LOG_DIR/b3_e2.log" 2>&1

PYTHONPATH=. /root/.pixi/bin/pixi run sim test_dt_v2_tiny \
  test_dt_v2_tiny.grid_id=b3_e2 \
  test_dt_v2_tiny.model_path=data/channel_generality/dt_v2_tiny/models/b3_e2/final_dt_v2.pth \
...
echo "=== [GPU3] DONE b3_e2 ===" | tee -a "$LOG_DIR/b3_e2.log"
