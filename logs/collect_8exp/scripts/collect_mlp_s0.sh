...
pixi run sim collect_data_v2 \
  collect_data_v2.model_path=/root/decision_transformer_slicing/data/channel_generality/dt_v2_mean_rwd/ppo_s0/best_model/best_model.zip \
  collect_data_v2.teacher_kind=single \
  collect_data_v2.teacher_id=ppo_s0_mlp \
  ...
  2>&1 | tee /root/decision_transformer_slicing/logs/collect_8exp/mlp_s0.log
echo "EXIT:$?"
